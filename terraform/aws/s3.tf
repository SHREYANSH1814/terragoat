...
resource "aws_s3_bucket" "operations" {
  ...
  replication_configuration {
    ...
    rules {
      ...
      status = "Enabled"
      destination {
        bucket        = aws_s3_bucket.operations_replica.arn
...
resource "aws_s3_bucket" "operations_replica" {
  bucket = "${local.resource_prefix.value}-operations-replica"
  acl    = "private"
  versioning {
    enabled = true
  }
...
resource "aws_iam_role" "replication_role" {
  name = "${local.resource_prefix.value}-replication-role"
...
resource "aws_iam_role_policy" "replication_policy" {
  name = "${local.resource_prefix.value}-replication-policy"
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = [
          "${aws_s3_bucket.operations_replica.arn}/*"
        ]
      }
    ]
  })
